...
options="--size=32 --run_infinitely --duration=1 --post-list=16"
...
taskset -c 0 ib_read_bw $options --ib-port=1 --port=$ib_p1_port2 $server &
...
taskset -c 0 ib_read_bw $options --ib-port=2 --port=$ib_p2_port1 $server &
taskset -c 0 ib_read_bw $options --ib-port=2 --port=$ib_p2_port2 $server &
