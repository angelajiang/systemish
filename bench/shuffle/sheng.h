...
struct Sheng {
  ...
  Sheng(std::vector<std::tuple<u32, u32, u8>>& trans_vec, u8 start_state_,
        u8 default_state) {
    // fill all transitions with default state
    for (u32 i = 0; i < 256; ++i) {
      transitions[i] = _mm_set1_epi8(default_state);
    }
    // fill in state transition for slot 'from' to point to 'to' for our
    // character transition c
    ...
    start_state = _mm_set1_epi8(
        start_state_);  // put everyone into start state - why not?
  }
