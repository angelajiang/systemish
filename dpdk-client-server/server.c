...
LL src_mac_arr[8] = {0x6c10bb211b00, 0x6d10bb211b00, 0x64d2bd211b00,
                     0x65d2bd211b00, 0xc8a610ca0568, 0xc9a610ca0568,
                     0xa2a610ca0568, 0xa3a610ca0568};

// xia-router0 xge0,1    xia-router1 xge0,1    xia-router0 xge2,3    xia-router1
// xge2,3
LL dst_mac_arr[8] = {0x36d3bd211b00, 0x37d3bd211b00, 0x44d7a3211b00,
...
void run_server(struct rte_mempool **l2fwd_pktmbuf_pool) {
  ...
  while (1) {
    ...
    // Lcores *cannot* wait for a particular number of packets from a port. If
    // we do this,
    // the port mysteriously runs out of RX descriptors.
    while (nb_rx_new < MAX_SRV_BURST && tries < 5) {
      nb_rx_new +=
          rte_eth_rx_burst(port_id, queue_id, &rx_pkts_burst[nb_rx_new],
    ...
    int nb_tx_new =
        rte_eth_tx_burst(port_id, queue_id, rx_pkts_burst, nb_rx_new);
