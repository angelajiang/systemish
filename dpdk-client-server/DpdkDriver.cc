...
namespace {
inline void timeTrace(const char* format, uint32_t arg0 = 0, uint32_t arg1 = 0,
...
// payload address.
#define payload_to_mbuf(payload)                               \
  reinterpret_cast<struct rte_mbuf*>(payload - ETHER_HDR_LEN - \
...
DpdkDriver::DpdkDriver(Context* context, int port)
  ...
  const char* argv[] = {"rc", "--file-prefix", nameBuffer, "-c", "1", "-n", "1",
                        NULL};
  ...
  if (numPorts <= portId) {
    throw DriverException(
        HERE, format("Ethernet port %u doesn't exist (%u ports available)",
                     portId, numPorts));
  }
...
int DpdkDriver::getHighestPacketPriority() {
  return highestPriorityAvail - lowestPriorityAvail;
}

// See docs in Driver class.
uint32_t DpdkDriver::getMaxPacketSize() { return MAX_PAYLOAD_SIZE; }

// See docs in Driver class.
uint32_t DpdkDriver::getPacketOverhead() {
  return ETHER_VLAN_HDR_LEN + ETHER_PACKET_OVERHEAD;
}

// See docs in Driver class.
void DpdkDriver::receivePackets(uint32_t maxPackets,
...
#define MAX_PACKETS_AT_ONCE 32
  ...
  uint32_t incomingPkts =
      rte_eth_rx_burst(portId, 0, mPkts, downCast<uint16_t>(maxPackets));
  if (incomingPkts > 0) {
#if TIME_TRACE
  ...
  for (uint32_t i = 0; i < totalPkts; i++) {
    ...
    if (unlikely(m->nb_segs > 1)) {
      RAMCLOUD_CLOG(WARNING, "Can't handle packet with %u segments; discarding",
                    m->nb_segs);
      rte_pktmbuf_free(m);
      continue;
    }
    ...
    if (ether_type == rte_cpu_to_be_16(ETHER_TYPE_VLAN)) {
      struct vlan_hdr* vlanHdr = reinterpret_cast<struct vlan_hdr*>(payload);
    ...
    if (!hasHardwareFilter) {
      ...
      if (ether_type != rte_cpu_to_be_16(NetUtil::EthPayloadType::RAMCLOUD)) {
        rte_pktmbuf_free(m);
        continue;
      }
    }
...
// See docs in Driver class.
void DpdkDriver::release(char* payload) {
...
// See docs in Driver class.
void DpdkDriver::releaseHwPacketBuf(Driver::Received* received) {
...
// See docs in Driver class.
void DpdkDriver::sendPacket(const Address* addr, const void* header,
                            uint32_t headerLen, Buffer::Iterator* payload,
                            int priority, TransmitQueueState* txQueueState) {
  ...
  if (unlikely(NULL == data)) {
    RAMCLOUD_CLOG(NOTICE, "rte_pktmbuf_append call failed; dropping packet");
    rte_pktmbuf_free(mbuf);
    return;
  }
  ...
  if (!memcmp(static_cast<const MacAddress*>(addr)->address, localMac->address,
              6)) {
    int ret = rte_ring_enqueue(loopbackRing, mbuf);
    if (unlikely(ret != 0)) {
      LOG(WARNING, "rte_ring_enqueue returned %d; packet may be lost?", ret);
      rte_pktmbuf_free(mbuf);
    }
  ...
  queueEstimator.packetQueued(physPacketLength, lastTransmitTime, txQueueState);
  PerfStats::threadStats.networkOutputBytes += physPacketLength;
}

// See docs in Driver class.
string DpdkDriver::getServiceLocator() { return locatorString; }

// See docs in Driver class.
uint32_t DpdkDriver::getBandwidth() { return bandwidthMbps; }
