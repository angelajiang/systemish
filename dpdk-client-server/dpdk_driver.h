...
#define MBUF_SIZE \
  (2048 + static_cast<uint32_t>(sizeof(struct rte_mbuf)) + RTE_PKTMBUF_HEADROOM)
...
class DpdkDriver : public Driver {
 public:
  ...
  virtual void sendPacket(const Address* addr, const void* header,
  ...
  static constexpr uint16_t PRIORITY_TO_PCP[8] = {
      1 << 13, 0 << 13, 2 << 13, 3 << 13, 4 << 13, 5 << 13, 6 << 13, 7 << 13};

  /// See docs in Driver class. The additional headroom space is used to
  /// store the packet buf type.
  typedef Driver::PacketBuf<MacAddress, MAX_PAYLOAD_SIZE, PACKETBUF_TYPE_SIZE>
      PacketBuf;
