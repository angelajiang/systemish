...
void run_client(int client_id, int *ht_log,
  ...
  LL dst_mac_arr[2][4] = {
      {0x6c10bb211b00, 0x6d10bb211b00, 0xc8a610ca0568, 0xc9a610ca0568},
  ...
  struct rte_mbuf *rx_pkts_burst[MAX_CLT_RX_BURST],
      *tx_pkts_burst[MAX_CLT_TX_BURST];
  ...
  while (1) {
    for (i = 0; i < MAX_CLT_TX_BURST; i++) {
      ...
      LL *tsc =
          (LL *)(rte_pktmbuf_mtod(tx_pkts_burst[i], char *) + hdr_size + 12);
      tsc[0] = rte_rdtsc();  // 48 -> 56
    }

    int nb_tx_new =
        rte_eth_tx_burst(port_id, queue_id, tx_pkts_burst, MAX_CLT_TX_BURST);
    nb_tx += nb_tx_new;
    ...
    while (1) {
      int nb_rx_new =
          rte_eth_rx_burst(port_id, queue_id, rx_pkts_burst, MAX_CLT_RX_BURST);
      if (nb_rx_new == 0) {
        break;
      }
      ...
      for (i = 0; i < nb_rx_new; i++) {
        ...
        LL *tsc =
            (LL *)(rte_pktmbuf_mtod(rx_pkts_burst[i], char *) + hdr_size + 12);
    ...
    if (unlikely(nb_tx >= 10000000)) {
      ...
      printf(
          "Lcore = %d, TX per sec = %f, Avg. latency = %.2f us | Fails = %lld, "
          "nb_rx = %lld\n",
