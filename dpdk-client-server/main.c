    ...
    .rx_adv_conf =
        {
            .rss_conf =
                {
                    .rss_key = NULL, .rss_hf = ETH_RSS_IPV4,
                },
        },
};
...
static const struct rte_eth_rxconf rx_conf = {
    .rx_thresh =
        {
            .pthresh = RX_PTHRESH, .hthresh = RX_HTHRESH, .wthresh = RX_WTHRESH,
        },
...
static const struct rte_eth_txconf tx_conf = {
    .tx_thresh =
        {
            .pthresh = TX_PTHRESH, .hthresh = TX_HTHRESH, .wthresh = TX_WTHRESH,
        },
...
int main(int argc, char **argv) {
  ...
  for (port_id = 0; port_id < nb_ports; port_id++) {
    ...
    // xia-router0/1 use an IO-Hub for PCIe devices, so NICs don't have a
    // NUMA-socket.
    int my_socket_id = is_client == 1 ? get_socket_id_from_macaddr(port_id)
                                      : rte_eth_dev_socket_id(port_id);
    int num_queues =
        is_client == 1 ? 3 : count_active_lcores_on_socket(my_socket_id);
    ...
    for (queue_id = 0; queue_id < num_queues; queue_id++) {
      ...
      ret = rte_eth_rx_queue_setup(port_id, queue_id, NUM_RX_DESC, my_socket_id,
                                   &rx_conf, mp);
      CPE2(ret < 0, "rte_eth_rx_queue_setup: %d, %u\n", ret, (unsigned)port_id);

      ret = rte_eth_tx_queue_setup(port_id, queue_id, NUM_TX_DESC, my_socket_id,
                                   &tx_conf);
